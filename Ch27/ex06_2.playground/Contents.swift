...
// 27-14 획득목록을 통한 클로저의 강한참조 순환 문제 해결
class Person {
    let name: String
    let hobby: String?
...
// 27-15 획득목록의 미소유참조로 인한 차후 접근 문제 발생
yagom = Person(name: "yagom", hobby: "eating")
...
yagom = nil // yagom is being deinitialized
// print(hana?.introduce())    // 오류 발생! 이미 메모리에서 해제된 인스턴스 참조(yagom) 시도

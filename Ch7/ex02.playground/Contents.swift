...
// 7-15 중첩 함수의 사용
func functionForMove0(_ shouldGoLeft: Bool) -> MoveFunc {
    func goRight(_ currentPosition: Int) -> Int {
        return currentPosition + 1
    }
    
    func goLeft(_ currentPostion: Int) -> Int {
...
let moveToZero0: MoveFunc = functionForMove(position > 0)

// 원점에 도착하면 반복문이 종료됩니다.
while position != 0 {
    print("\(position)...")
    position = moveToZero0(position)
}
print("원점 도착!")
